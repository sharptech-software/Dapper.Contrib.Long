...
namespace Dapper.Contrib.Long
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction? transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (type.IsInterface)
            {
                if (!(connection.Query(sql, dynParams, transaction, commandTimeout: commandTimeout).FirstOrDefault() is IDictionary<string, object> res))
                {
                    return null;
                }
        ...
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, IDbTransaction? transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            {
                var result = connection.Query(sql, null, transaction, commandTimeout: commandTimeout);
                var list = new List<T>();
                var includeRowVersion = typeof(IVersionedEntity).IsAssignableFrom(type) && IsPostgreSql(connection);
        ...
        /// <summary>
        /// Returns a list of entities from table "Ts" that match the complete where clause provided.
        /// </summary>
        /// <typeparam name="T">Interface or type to create and populate</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        ...
        /// <param name="transaction">The transaction to run under, null (the default) if none</param>
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>An enumerable collection of T</returns>
        public static IEnumerable<T> GetByCompleteWhereClause<T>(this IDbConnection connection, string completeWhereClause, object? param = null, IDbTransaction? transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            ...
            {
                var result = connection.Query(sql, param, transaction, commandTimeout: commandTimeout);
                var list = new List<T>();
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
        ...
        /// <summary>
        /// Returns the key column name for table "Ts".
        /// </summary>
        ...
        public static string GetKeyName<T>() where T : class
        {
            ...
            var keyProperties = KeyPropertiesCache(type).ToList();  //ToList() due to issue #418
            ...
            if (keyProperties.Count > 1)
                throw new ArgumentException("GetKeyName only supports entities with a single key property");
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction? transaction = null, int? commandTimeout = null, bool overrideRowVersion = false) where T : class
        {
            ...
            var returnRowVersion = typeof(IVersionedEntity).IsAssignableFrom(type) && IsPostgreSql(connection);
            var checkRowversion = returnRowVersion && overrideRowVersion == false;
        ...
        /// <summary>
        /// Delete entity in table "Ts" by key value, without needing a full entity instance.
        /// </summary>
        /// <typeparam name="T">Type of entity</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        ...
        /// <param name="commandTimeout">Number of seconds before command execution timeout</param>
        /// <returns>true if deleted, false if not found</returns>
        public static bool DeleteById<T>(this IDbConnection connection, long id, IDbTransaction? transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);

            var keyProperties = KeyPropertiesCache(type).ToList();  //ToList() due to issue #418, work on a list copy
            var explicitKeyProperties = ExplicitKeyPropertiesCache(type);
            if (keyProperties.Count == 0 && explicitKeyProperties.Count == 0)
                throw new ArgumentException("Entity must have at least one [Key] or [ExplicitKey] property");
            ...
            keyProperties.AddRange(explicitKeyProperties);
            if (keyProperties.Count > 1)
                throw new ArgumentException("Delete by id only supports entities with a single key property");
            ...
            sb.AppendFormat("delete from {0} where ", name);
            var adapter = GetFormatter(connection);
            adapter.AppendColumnNameEqualsValue(sb, keyProperty.Name);  //fix for issue #336

            var parameters = new DynamicParameters();
            parameters.Add(keyProperty.Name, id);

            var deleted = connection.Execute(sb.ToString(), parameters, transaction, commandTimeout);
            return deleted > 0;
        }

        /// <summary>
        /// Delete entities in table "Ts" by a set of key values, without needing full entity instances.
        /// </summary>
        /// <typeparam name="T">Type of entity</typeparam>
        /// <param name="connection">Open SqlConnection</param>
        ...
        public static bool DeleteByIds<T>(this IDbConnection connection, IEnumerable<long> ids, IDbTransaction? transaction = null, int? commandTimeout = null) where T : class
        {
            if (ids == null)
            ...
            keyProperties.AddRange(explicitKeyProperties);
            if (keyProperties.Count > 1)
                throw new ArgumentException("DeleteByIds only supports entities with a single key property");
            ...
            var deleted = connection.Execute(sb.ToString(), new { ids = idList }, transaction, commandTimeout);
            return deleted > 0;
        }
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
    ...
    public class ConcurrencyConflictException : Exception
    {
        ...
        public ConcurrencyConflictException(Type entityType, object id, long? expectedVersion, long? actualVersion)
            : base($"Concurrency conflict on {entityType.Name} with id {id}. " + (actualVersion == null ? "No RowVersion was supplied." : "The row has been modified since it was loaded."))
        {
